...
class NoSQLQueryResultViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = results?[(indexPath as NSIndexPath).section]
        let modelDictionary: [AnyHashable: Any] = model!.dictionaryValue
        let attributeKey = table?.tableAttributeName!(table!.orderedAttributeKeys[(indexPath as NSIndexPath).row])
        cell.attributeNameLabel.text = attributeKey!
        cell.attributeValueLabel.text = "\(modelDictionary[(table?.orderedAttributeKeys[(indexPath as NSIndexPath).row])!]!)"
        if (!loading) && (paginatedOutput?.lastEvaluatedKey != nil) && (indexPath as NSIndexPath).section == self.results!.count - 1 {
    ...
    func updateItemForIndexPath(_ indexPath: IndexPath) {
        let item: AWSDynamoDBObjectModel = self.results![(indexPath as NSIndexPath).section]
    ...
    func removeItemForIndexPath(_ indexPath: IndexPath) {
        table?.removeItem?(self.results![(indexPath as NSIndexPath).section], completionHandler: {(error: NSError?) -> Void in
            if let error = error { // Handle error if any
                var errorMessage = "Error Occurred: \(error.localizedDescription)"
                if (error.domain == AWSServiceErrorDomain && error.code == AWSServiceErrorType.accessDeniedException.rawValue) {
